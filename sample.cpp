...
const char *WINDOWTITLE = "Carousel Animation - Jack Hart";
...
bool	outView;				// Whether we're inside or outside view
...
void initHorses(float rockFreq, float offset) {

	// Reset transformation matrix so our horses' transformations don't stack
	glLoadIdentity();
	...
	float offsetTime = Time - offset;

	float xPos = 2 * sin(1 * (F_2_PI * offsetTime));
	float zPos = 2 * cos(1 * (F_2_PI * offsetTime));

	float yRot = offsetTime * 360;
	float zRot = 25 * cos(rockFreq * (F_2_PI * offsetTime));

	float rock = 0.5 * sin(rockFreq * (F_2_PI * offsetTime)) + 0.5;

	// Transform
	glTranslatef(0, rock, 0);
...
Display( )
{
	...
	glLoadIdentity( );
	if( NowProjection == ORTHO && outView )
	...
	glNewList( CircleList, GL_COMPILE );
		...
		glBegin( GL_LINE_LOOP );
			...
			for (int i = 0; i < resolution; i++) {
					glVertex3f(sin(((float)i/resolution) * F_2_PI) * r, 0, cos(((float)i / resolution) * F_2_PI) * r);
			}
